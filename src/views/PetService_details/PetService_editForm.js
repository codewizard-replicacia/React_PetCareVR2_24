...
const EditPetServiceForm = () => {
  ...
  useEffect(() => {
    const fetchPetServiceById = async () => {
      ...
      const jsonResp = await petServiceResponse.json();
  ...
  const submitForm = async () => {
    ...
    const resp = await makeApiCall(
      `${BASE_URL}${PATH_PETSERVICE}(${formData.ServiceId})`,
      'PATCH',
            JSON.stringify({
        ...otherData,
    ...
    if (resp.ok) {
      snackbar.enqueueSnackbar('Successfully updated PetService', {
        variant: 'success',
      });
      navigate({ pathname: '/petServices' });
    } else {
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <div className="action-buttons">
              ...
              <Button
                ...
                variant="contained"
                color="secondary"
                onClick={() => navigate({ pathname: '/petServices' })}
              >
                Cancel
              </Button>
            </div>
...
export default EditPetServiceForm;
